...
// ["manager", vault_address, "market", market_id]
#[account]
pub struct MarketConfig {
  pub bump: u8,
  pub cap: u64,                   // Supply cap for this market
  pub enabled: bool,              // Whether market is enabled
  pub removable_at: Option<u64>,  // Timestamp when market can be removed
  pub pending_cap: Option<u64>, // Pending cap change
}

// Queue Account - Stores supply and withdraw queues
// (Multiple accounts, one per queue)
// Queue Account - Single queue state per vault
// ["manager", vault_address, "queue"]
